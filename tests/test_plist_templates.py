...
def _template(name):
    p = _ROOT / f"com.jobsentinel.{name}.plist.template"
    return p if p.exists() else _ROOT / "share" / f"com.jobsentinel.{name}.plist.template"


_TEMPLATES = {"daily": _template("daily"), "dashboard": _template("dashboard")}
