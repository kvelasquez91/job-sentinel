...
def _template_path(repo_dir: str, agent: str) -> str:
    """Template location: repo root (public tree) or share/ (owner tree)."""
    name = f"com.jobsentinel.{agent}.plist.template"
    for cand in (os.path.join(repo_dir, name),
                 os.path.join(repo_dir, "share", name)):
...
def render(config: dict, repo_dir: str, home: str, claude_dir: str) -> dict:
    ...
    out = {}
    for agent in AGENTS:
        template = _template_path(repo_dir, agent)
